...
impl NativePdfConverter {
    ...
    fn extract_text_with_system_tools(&self, pdf_data: &[u8]) -> FlowyResult<String> {
        info!("Attempting to extract text using system tools");
...
impl DocumentConverter for NativePdfConverter {
    ...
    async fn convert(&self, task: &ConversionTask) -> FlowyResult<ConversionResult> {
        ...
        info!("Starting native PDFium conversion for file: {}", task.source_path);
        ...
        // 提取图像
        let extracted_images = converter.extract_images(&pdf_data)?;

        // 转换为HTML格式
        let html_content = converter.text_to_html(&extracted_text);
